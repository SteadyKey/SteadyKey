...
function toNumberArray(u: Uint8Array): number[] {
  return Array.from(u);
}

/** Convert number[] to Uint8Array */
function fromNumberArray(arr: number[] | undefined): Uint8Array | null {
  if (!arr) return null;
...
async function createStretchedString(raw: Uint8Array): Promise<string> {
...
export async function getOrCreateRawMachineKey(): Promise<Uint8Array> {
  const stored = await storageGet<{ [RAW_KEY_STORAGE]?: number[] }>([RAW_KEY_STORAGE]);
...
export async function getStretchedMachineKeyForDisplay(): Promise<string> {
  const stored = await storageGet<{ [STRETCHED_STORAGE]?: string; [RAW_KEY_STORAGE]?: number[] }>([STRETCHED_STORAGE, RAW_KEY_STORAGE]);
...
export async function deleteMachineKey(): Promise<void> {
